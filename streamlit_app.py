# streamlit_app.py  ─ Streamlit Cloud 배포용
# stream=False 방식으로 완전 재작성 (스트림 끊김 오류 해결)
# ============================================================
...
# ── 0. Secrets → 환경변수 주입 (최우선) ───────────────────────
def _load_secrets():
    for key in ("HF_TOKEN","HF_MODEL_ID","HF_MAX_TOKENS","HF_TEMPERATURE","LLM_ENGINE"):
        try:
...
.main-title{font-size:2.2rem;font-weight:900;
  background:linear-gradient(135deg,#00d4ff,#7c3aed 50%,#f59e0b);
...
.metric-card{background:linear-gradient(135deg,#0f172a,#1e293b);
  border:1px solid #334155;border-radius:12px;padding:1rem 1.2rem;
...
.metric-value{font-size:1.7rem;font-weight:900;margin:.15rem 0;font-family:'JetBrains Mono',monospace;}
.metric-sub{font-size:.78rem;color:#64748b;}
.up-color{color:#22c55e;}.down-color{color:#ef4444;}.neu-color{color:#f59e0b;}
...
.tag{display:inline-block;padding:.12rem .5rem;border-radius:6px;font-size:.76rem;font-weight:600;margin:.1rem;}
...
.gemma-box{background:linear-gradient(135deg,#0f172a,#1a0a2e);
  border:1px solid #7c3aed55;border-radius:12px;padding:1.2rem 1.4rem;margin-top:.8rem;}
.gemma-header{font-size:.8rem;color:#a78bfa;font-weight:700;
  text-transform:uppercase;letter-spacing:.08em;margin-bottom:.6rem;}

.secret-ok{background:#16a34a18;border:1px solid #22c55e44;border-radius:8px;
  padding:.5rem .8rem;font-size:.82rem;color:#22c55e;}
.secret-err{background:#dc262618;border:1px solid #ef444444;border-radius:8px;
  padding:.5rem .8rem;font-size:.82rem;color:#ef4444;}
...
def tag(label, active):
    return f'<span class="tag {"tag-on" if active else "tag-off"}">{label}</span>'
...
def get_token():
    return (st.session_state.get("hf_token_override") or _RAW_TOKEN or "")
...
# ── 4. Gemma-4 자동 초기화 ────────────────────────────────────
@st.cache_resource(show_spinner="🤖 Gemma-4 연결 확인 중…")
def init_gemma(token: str):
    if not token:
        return {"success":False,"error":"토큰 없음","reply":""}
    return test_connection(token)

if _RAW_TOKEN and not st.session_state.get("gemma_init_done"):
    _res = init_gemma(_RAW_TOKEN)
    st.session_state.update({
        "gemma_ok":        _res["success"],
        "gemma_init_done": True,
        "gemma_reply":     _res.get("reply",""),
        "gemma_error":     _res.get("error",""),
    })
    if _res["success"]:
        os.environ["HF_TOKEN"] = _RAW_TOKEN
...
def plot_candle(raw, result):
    ...
    c2=feat["close"]; bm=c2.rolling(20).mean(); bs=c2.rolling(20).std()
    fig.add_trace(go.Scatter(x=fdates,y=bm+2*bs,line=dict(color="#475569",width=.8,dash="dash"),
        name="BB상단",opacity=.5),row=1,col=1)
    fig.add_trace(go.Scatter(x=fdates,y=bm-2*bs,line=dict(color="#475569",width=.8,dash="dash"),
        name="BB하단",fill="tonexty",fillcolor="rgba(71,85,105,0.06)",opacity=.5),row=1,col=1)
    ...
    fig.add_trace(go.Bar(x=df["date"],y=df["volume"],marker_color=colors,
        name="거래량",opacity=.7),row=2,col=1)
...
def plot_gauge(pred_ret, conf_band):
    ...
    fig=go.Figure(go.Indicator(
        ...
        gauge={"axis":{"range":[-15,15],"tickcolor":"#475569","tickfont":{"color":"#94a3b8","size":10}},
    ...
    fig.add_annotation(text=f"신뢰구간: {pred_ret-conf_band:+.2f}% ~ {pred_ret+conf_band:+.2f}%",
        xref="paper",yref="paper",x=.5,y=-.1,showarrow=False,
...
def plot_scan_bar(df):
    ...
    fig=go.Figure(go.Bar(x=df["name"],y=df["pred_ret_pct"],marker_color=colors,
...
# ── 6. Gemma 결과 박스 렌더링 ──────────────────────────────────
def render_gemma_box(title: str, content: str):
    """Gemma-4 결과를 스타일 박스 안에 마크다운으로 렌더"""
    st.markdown(f"""
<div class="gemma-box">
  <div class="gemma-header">🤖 {title} &nbsp;·&nbsp; {HF_MODEL}</div>
...
with st.sidebar:
    ...
    if gemma_ok:
        st.markdown(
            '<div class="secret-ok">✅ Gemma-4 연결됨<br>'
            '<span style="font-size:.75rem;opacity:.8">Secrets에서 토큰 자동 로드</span></div>',
            unsafe_allow_html=True)
        reply_preview = st.session_state.get("gemma_reply","")
        if reply_preview:
            with st.expander("연결 확인 메시지"):
                st.caption(reply_preview)
    elif _RAW_TOKEN:
        err = st.session_state.get("gemma_error","알 수 없는 오류")
        st.markdown(
            f'<div class="secret-err">❌ 연결 실패<br>'
            f'<span style="font-size:.75rem">{err[:80]}</span></div>',
            unsafe_allow_html=True)
        with st.expander("🔑 토큰 재입력"):
            mt = st.text_input("HF Token",type="password",placeholder="hf_xxxxxxxxxxxxx")
            if st.button("재연결", use_container_width=True) and mt:
                with st.spinner("재연결 중…"):
                    res2 = test_connection(mt)
                if res2["success"]:
                    st.session_state.update({"hf_token_override":mt,
                        "gemma_ok":True,"gemma_error":""})
                    os.environ["HF_TOKEN"] = mt
                    st.success("✅ 연결 성공!")
                    st.rerun()
                else:
        ...
        st.markdown(
            '<div class="secret-err">⚠️ Secrets 미설정<br>'
            '<span style="font-size:.75rem">App Settings → Secrets에<br>HF_TOKEN 등록 필요</span></div>',
            unsafe_allow_html=True)
        with st.expander("🔑 토큰 직접 입력 (임시)"):
            mt = st.text_input("HF Token",type="password",placeholder="hf_xxxxxxxxxxxxx")
            if st.button("연결", use_container_width=True) and mt:
                with st.spinner("연결 테스트 중…"):
                    res3 = test_connection(mt)
                if res3["success"]:
                    st.session_state.update({"hf_token_override":mt,
                        "gemma_ok":True,"gemma_init_done":True,
                        "gemma_reply":res3["reply"]})
                    os.environ["HF_TOKEN"] = mt
                    st.success("✅ 연결 성공!")
                    st.rerun()
                else:
...
""", language="toml")
    ...
    gemma_ok = st.session_state.get("gemma_ok", False)  # 재확인

    if "단일" in mode:
        ticker_query = st.text_input("종목명 또는 코드",
                                      placeholder="삼성전자 / 005930 / 하이닉스")
        use_rt  = st.toggle("실시간 시세", value=True)
        use_llm = st.toggle("Gemma-4 분석", value=gemma_ok, disabled=not gemma_ok)
        ...
        run_btn=False; use_llm=True; use_rt=True
        scan_n=150; top_n=10; ticker_query=""

    st.markdown("**⚠️ 투자 주의**\n본 예측은 참고용이며 투자 손실에 책임지지 않습니다.")
...
if "단일" in mode:
    if run_btn and ticker_query:
        ...
        if r is None:
            st.error("❌ 종목을 찾을 수 없거나 데이터 부족합니다.")
        else:
            ...
            c1.markdown(mc("예측 수익률",f"{pr:+.2f}%",f"± {cb:.2f}%",col_c),unsafe_allow_html=True)
            c2.markdown(mc("예측 상승률",f"+{r['pred_up_pct']:.2f}%",f"과거평균 {r['up_hist_avg'] or '-'}%","up-color"),unsafe_allow_html=True)
            c3.markdown(mc("예측 하락률",f"-{r['pred_dn_pct']:.2f}%",f"과거평균 -{r['dn_hist_avg'] or '-'}%","down-color"),unsafe_allow_html=True)
            c4.markdown(mc("15% 급등 확률",f"{r['prob_up15']*100:.1f}%",f"과거 {r['pos_count']}회","neu-color"),unsafe_allow_html=True)
            c5.markdown(mc("급등 점수",f"{r['score']:.1f}","/ 100",col_c),unsafe_allow_html=True)
            ...
            with ga: st.plotly_chart(plot_gauge(pr,cb),use_container_width=True)
            with ca: st.plotly_chart(plot_candle(r["raw"],r),use_container_width=True)
            ...
            with ic:
                ...
                st.metric("CV AUC",           f"{r['cv_auc']:.3f}" if r['cv_auc'] else "N/A")

            # Gemma-4 분석
            if use_llm and gemma_ok:
                st.markdown("---")
                with st.spinner("🤖 Gemma-4 분석 리포트 생성 중… (10~30초)"):
                    try:
                        tok = get_token()
                        report = analyze_stock_with_gemma(
                            r, tok,
                            max_tokens=HF_MAX_TOKENS,
                            temperature=HF_TEMPERATURE,
                        )
                        render_gemma_box("Gemma-4 AI 분석 리포트", report)
                    except Exception as e:
                        st.error(f"❌ Gemma-4 오류: {e}")
            elif use_llm and not gemma_ok:
                st.info("💡 Secrets에 HF_TOKEN을 등록하면 AI 분석이 자동 활성화됩니다.")

            with st.expander("🤖 ML 모델 정보"):
                mm1,mm2,mm3=st.columns(3)
                mm1.metric("CV AUC",    r["cv_auc"] or "N/A")
                mm2.metric("CV Reg",    r["cv_reg"] or "N/A")
                mm3.metric("15%↑ 발생률",f"{r['base_rate']*100:.1f}%")
...
# ══════════════════════════════════════════════════════════════
    ...
    if run_btn:
        ...
        def pcb(i,total,name):
            if total>0:
                pb.progress(min(int(i/total*100),100),text=f"분석 중: {name} ({i}/{total})")
        df=agent.scan_top(max_stocks=scan_n,top_n=top_n,use_realtime=use_rt,progress_cb=pcb)
        pb.empty()
        ...
        if df is None or len(df)==0:
            ...
            st.markdown("#### 📋 상세 결과")
            disp=df[["rank","name","market","close","pred_ret_pct","pred_up_pct",
                      "pred_dn_pct","conf_band","prob_up15","cv_auc","score",
                      "vol_ratio20","rsi14","vol_explosion","breakout_flag","near_52w_high"]].copy()
            disp.columns=["순위","종목명","시장","종가","예측수익률%","상승%","하락%","신뢰±",
                           "15%확률","CV_AUC","점수","거래량비율","RSI14","거래량폭발","이평돌파","52주신고가"]

            def hl(s):
                try: v=float(s); return "color:#22c55e;font-weight:700" if v>=0 else "color:#ef4444;font-weight:700"
                except: return ""

            st.dataframe(
                disp.style.applymap(hl,subset=["예측수익률%","상승%"])
                .format({"종가":"{:,.0f}","예측수익률%":"{:+.2f}","상승%":"+{:.2f}",
                         "하락%":"-{:.2f}","신뢰±":"±{:.2f}","15%확률":"{:.1%}",
                         "점수":"{:.1f}","거래량비율":"{:.2f}x","RSI14":"{:.0f}",
            ...
            if use_llm and gemma_ok:
                st.markdown("---")
                with st.spinner("🤖 Gemma-4 종합 시장 판단 생성 중… (10~30초)"):
                    try:
                        tok=get_token()
                        report=analyze_top10_with_gemma(
                            df,tok,
                            max_tokens=HF_MAX_TOKENS,
                            temperature=HF_TEMPERATURE,
                        )
                        render_gemma_box("Gemma-4 종합 시장 판단",report)
                    except Exception as e:
                        st.error(f"❌ Gemma-4 오류: {e}")
            ...
            if st.button("상세 분석",type="secondary"):
                ...
                if r2:
                    ...
                    d1.metric("예측 수익률",  f"{r2['pred_ret_pct']:+.2f}%",f"±{r2['conf_band']:.2f}%")
                    ...
                    if use_llm and gemma_ok:
                        with st.spinner("🤖 Gemma-4 개별 분석 중…"):
                            try:
                                tok=get_token()
                                rep2=analyze_stock_with_gemma(
                                    r2,tok,
                                    max_tokens=HF_MAX_TOKENS,
                                    temperature=HF_TEMPERATURE,
                                )
                                render_gemma_box("Gemma-4 개별 분석",rep2)
                            except Exception as e:
                                st.error(f"❌ Gemma-4 오류: {e}")
    else:
        st.info(f"👈 설정 후 **스캔 시작**을 눌러주세요.")
        st.markdown("> 50종목→5~10분 │ 150종목→15~25분 │ 300종목→30~50분")
...
# ══════════════════════════════════════════════════════════════
    ...
    st.caption(f"모델: `{HF_MODEL}` │ max_tokens={HF_MAX_TOKENS} │ temp={HF_TEMPERATURE}")

    if not gemma_ok:
        st.warning("⚠️ Gemma-4가 연결되지 않았습니다. Secrets에 HF_TOKEN을 등록해 주세요.")
        st.code(
            'LLM_ENGINE      = "hf_api"\n'
            'HF_TOKEN        = "hf_your_token_here"\n'
            'HF_MODEL_ID     = "google/gemma-4-26B-A4B-it"\n'
    ...
    selected_q=None
    for qc,qq in zip(st.columns(4),quick_qs):
        with qc:
    ...
    if question:
        ...
        # ── Gemma-4 호출 (stream=False, 스피너 표시) ──────────
        with st.spinner("🤖 Gemma-4 답변 생성 중… (10~20초)"):
            try:
                tok=get_token()
        ...
        st.markdown(f'<div class="chat-ai">🤖 &nbsp;{reply}</div>',
                    unsafe_allow_html=True)
