...
FEATURE_COLS = [
    ...
    "bb_pos","bb_width","atr_ratio",
    "stoch_k","stoch_d","obv_slope","cci14",
...
def make_features(df: pd.DataFrame, target_pct: float = 0.15) -> pd.DataFrame:
    ...
    for n in [3,5,10,20,60,120]:
        ma = c.rolling(n).mean()
        d[f"ma{n}"] = ma; d[f"dist_ma{n}"] = c / ma - 1
    for n in [5,20]:
    ...
    body = (c-o).abs(); rng = (h-l).clip(lower=1e-9)
    d["range_pct"]    = rng / c
    ...
    d["stoch_k"], d["stoch_d"] = _stoch(h,l,c)
    ...
    d["price_compress"] = ((c.rolling(5).max()-c.rolling(5).min()) < atr14).astype(int)
